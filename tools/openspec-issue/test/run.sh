...
python3 -c 'import sys; p=sys.argv[1]; s=open(p).read().replace("<!-- openspec:section:tasks:end -->", ""); open(p,"w").write(s)' "$store7/issues/1/body"
...
python3 -c 'import sys; p=sys.argv[1]; s=open(p).read().replace("\"lifecycle\":\"proposed\"", "\"lifecycle\":\"implementing\""); open(p,"w").write(s)' "$store9/issues/$cn/body"
...
found201="$( ( export OPENSPEC_ISSUE_PER_PAGE=50; run "$store18" find target-change 2>/dev/null ) )"
[[ "$found201" == "201" ]] && ok "pagination finds issue #201 beyond page limit" || no "pagination find (#$found201)"
dupout="$( ( export OPENSPEC_ISSUE_PER_PAGE=50; run "$store18" create --name target-change --title t --body-file "$BODY" 2>&1; echo "rc=$?" ) )"
