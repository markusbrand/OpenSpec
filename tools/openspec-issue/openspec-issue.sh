...
gh_call() {
  ...
  local out rc _repo=() _rline
  while IFS= read -r _rline || [[ -n "$_rline" ]]; do
    [[ -n "$_rline" ]] && _repo+=("$_rline")
  done < <(repo_args)
  set +e
...
is_date() {
  ...
  if command -v node >/dev/null 2>&1; then
    node -e 'const s=process.argv[1]; const [y,m,d]=s.split("-").map(Number); const dt=new Date(Date.UTC(y,m-1,d)); if (dt.getUTCFullYear()!==y||dt.getUTCMonth()+1!==m||dt.getUTCDate()!==d) process.exit(1);' "$1" 2>/dev/null && return 0
  fi
  return 1
}
...
assert_issues_wellformed() {
  ...
  done < <(printf '%s' "$json" | jq -r '.[].number')
}

filter_issue_numbers() {
  # filter_issue_numbers <changeName> <issues-json> ; prints matching issue numbers
  local name="$1" json="$2"
  jq -c '.[]' <<<"$json" 2>/dev/null | while IFS= read -r item || [[ -n "$item" ]]; do
    [[ -z "$item" ]] && continue
    local body num cname
    body="$(jq -r '.body // ""' <<<"$item")"
    num="$(jq -r '.number' <<<"$item")"
    cname="$(printf '%s' "$body" | extract_metadata | jq -r '.changeName // ""' 2>/dev/null || true)"
    if [[ "$cname" == "$name" ]]; then
      printf '%s\n' "$num"
    fi
  done
...
cmd_find() {
  ...
  local nums=() _num_line
  while IFS= read -r _num_line || [[ -n "$_num_line" ]]; do
    [[ -n "$_num_line" ]] && nums+=("$_num_line")
...
cmd_create() {
  ...
  local existing=() _ex_line
  while IFS= read -r _ex_line || [[ -n "$_ex_line" ]]; do
    [[ -n "$_ex_line" ]] && existing+=("$_ex_line")
...
cmd_set_section() {
  ...
  awk -v s="$sec" -v nf="$work/new" '
    ...
    {sub(/\r$/,"")}
    $0 ~ ("<!-- openspec:section:" s ":start -->") {
      print
      printf "%s", newc
      skip=1
      next
    }
    $0 ~ ("<!-- openspec:section:" s ":end -->") {
      skip=0
      print
      next
    }
...
_validate_issue() {
  ...
  local labels=() _lbl_line
  while IFS= read -r _lbl_line || [[ -n "$_lbl_line" ]]; do
    [[ -n "$_lbl_line" ]] && labels+=("$_lbl_line")
  done < <(issue_labels "$num")
  printf '%s\n' ${labels[@]+"${labels[@]}"} | grep -qxF "$DISCOVERY_LABEL" \
    || die "$EX_VALIDATE" "issue #$num missing '$DISCOVERY_LABEL' label"
  for l in "${LIFECYCLE_LABELS[@]}"; do
    if printf '%s\n' ${labels[@]+"${labels[@]}"} | grep -qxF "$l"; then
