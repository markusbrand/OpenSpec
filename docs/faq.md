...
Two steps. Upgrade the package (`npm install -g github:markusbrand/OpenSpec --allow-git=all`), then run `openspec update` inside each project to refresh the generated skills and commands.

### How do I uninstall OpenSpec?

There's no uninstall command, because it's just a global package plus files in your project. Remove the package (`npm uninstall -g @markusbrand/openspec`), and optionally delete the `openspec/` directory and the generated tool files. Step-by-step, including what's safe to keep, is in [Installation: Uninstalling](installation.md#uninstalling).
...
- **GitHub Issues:** [github.com/markusbrand/OpenSpec/issues](https://github.com/markusbrand/OpenSpec/issues)
